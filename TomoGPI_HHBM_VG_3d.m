function [MeanTime,fh,ve,vxi,vz,v_erf_l2,v_erg_l2,v_erz_l2,v_erf_l1,v_erg_l1,v_erz_l1] = ...
                                          TomoGPI_HHBM_VG_3d(H,g,itermax,fh0,f,l,snr,niter_gradient,az0,bz0)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% epsilon_imposed_list=[0.1 0.3 0.8 1.35];
% epsilon_imposed_list=[0.01 0.05 0.3 1.35];
% k=1;
% epsilon_list=epsilon_imposed_list./sqrt(k);
% alpha_list=2+epsilon_list.^2;
% beta_list=sqrt(k).*epsilon_list.*(1+epsilon_list.^2);

% az0=alpha_list(1)*ones(M,N,R);
% bz0=beta_list(1)*ones(M,N,R);
% nMb=M/2;
% nNb=N/2;
% nRb=R/2;
% for ii=1:1:(l-1)
...
%         list=4;
%     end
%     az0(1:nMb,1:nNb,1:nRb)=alpha_list(list)*ones(nMb,nNb,nRb);
%     bz0(1:nMb,1:nNb,1:nRb)=beta_list(list)*ones(nMb,nNb,nRb);
%     nMb=nMb/2;
%     nNb=nNb/2;
%     nRb=nRb/2;
% end
% az0(1:nMb,1:nNb,1:nRb)=alpha_list(end)*ones(nMb,nNb,nRb);
% bz0(1:nMb,1:nNb,1:nRb)=beta_list(end)*ones(nMb,nNb,nRb);
% az0=az0(:);
% bz0=bz0(:);
...
% reconstruction
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
disp('Starting iterations for VG prior')
disp('...')
...
for iter=1:1:itermax
    ...
    vz=((az0-3/2)+sqrt((az0-3/2).^2+bz0.*zh(:).^2))./(2*bz0);
    
    % ve
    deltag=g(:)-H*fh(:);
    %ve=(be0+0.5*deltag.^2)./(ae0+1.5);
    ve=((ae0-3/2)+sqrt( (ae0-3/2).^2+be0.*deltag.^2))./(2*be0);
    
    % vxi
    Dzh=ihaar3_GPU(zh,M,N,R,l);
    er_f=fh(:)-Dzh(:);
    %vxi=(bxi0+0.5*er_f.^2)./(axi0+1.5);
    vxi=((axi0-3/2)+sqrt((axi0-3/2).^2+2*bxi0.*er_f.^2))./(2*bxi0);
...
disp('Finished iterations for VG prior');
