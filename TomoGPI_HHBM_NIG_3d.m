function [MeanTime,fh,ve,vxi,vz,v_erf_l2,v_erg_l2,v_erz_l2,v_erf_l1,v_erg_l1,v_erz_l1] = ...
                           TomoGPI_HHBM_NIG_3d(H,g,itermax,fh0,f,l,snr,niter_gradient,az0,bz0)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% reconstruction
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
disp('Starting iterations for NIG prior')
disp('...')
...
for iter=1:1:itermax
    ...
    vz=(-2+sqrt(4+az0.*(bz0+zh(:).^2)))./az0;
    
    % ve
    deltag=g(:)-H*fh(:);
    %ve=(be0+0.5*deltag.^2)./(ae0+1.5);
    ve=(-2+sqrt(4+ae0.*(be0+deltag.^2)))./ae0;
    
    % vxi
    Dzh=ihaar3_GPU(zh,M,N,R,l);
    er_f=fh(:)-Dzh(:);
    %vxi=(bxi0+0.5*er_f.^2)./(axi0+1.5);
    vxi=(-2+sqrt(4+axi0.*(bxi0+er_f.^2)))./axi0;
...
disp('Finished iterations for NIG prior');
