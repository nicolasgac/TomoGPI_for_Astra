...
y=flatten(y);
z=flatten(z);
save('temp.mat','z','-v7.3');
clear z;
coord = single([x;y]);
clear x;
clear y;
load('temp.mat');
coord= single([coord;z]);


clear z;
p = single(zeros([n n n]));
p = flatten(p);
...
for k = 1:size(ellipse,1)    
   ...
   idx = find((coordp(1,:)-x0).^2./asq + (coordp(2,:)-y0).^2./bsq + ...
              (coordp(3,:)-z0).^2./csq <= 1);
...
fid = fopen(['shepp_logan_' num2str(n) '.v'], 'w');
fwrite(fid, p, 'single');
fclose(fid);
...
e = modified_shepp_logan;
e(:,1) = single([1 -.98 -.02 -.02 .01 .01 .01 .01 .01 .01]);
...
%        -----------------------------------------------------------------
e =    single([  1  .6900  .920  .810      0       0       0      0      0      0
...
%         A      a     b     c     x0      y0      z0    phi  theta    psi
%        -----------------------------------------------------------------
e =    single([  1  .6900  .920  .900      0       0       0      0      0      0
